...
(function(){
  ...
  listen(canvas,'pointerdown',e=>{if(e.pointerType==='touch')return;if(e.button===0){mouseDown=true;input.fire=true;audio.unlock();canvas.focus({preventScroll:true});const p=coordinates(e);input.aimX=p.x;input.aimY=p.y;}});
  listen(window,'pointerup',e=>{if(e.pointerType!=='touch'&&e.button===0){mouseDown=false;input.fire=false;}});
  listen(window,'pointercancel',()=>{mouseDown=false;input.fire=false;});
  listen(canvas,'pointermove',e=>{if(e.pointerType==='touch')return;const p=coordinates(e);input.aimX=p.x;input.aimY=p.y;});
  ...
  if(coarse){ui.play.classList.add('touch-play');ui.play.appendChild(document.getElementById('touchControls'));}
  function stick(element,kind){let pointer=null;const knob=element.querySelector('i');
    ...
    listen(element,'pointerdown',e=>{if(pointer!==null)return;pointer=e.pointerId;element.setPointerCapture(pointer);audio.unlock();move(e);e.preventDefault();});
    listen(element,'pointermove',e=>{if(e.pointerId===pointer){move(e);e.preventDefault();}});
  ...
  document.querySelectorAll('[data-touch]').forEach(button=>listen(button,'pointerdown',e=>{e.preventDefault();audio.unlock();if(game.mode!=='playing')return;const action=button.dataset.touch;if(action==='swap')input.swap='next';else input[action]=true;}));
  function collectInput(){
    input.mx=(keys.has('KeyD')||keys.has('ArrowRight')?1:0)-(keys.has('KeyA')||keys.has('ArrowLeft')?1:0)+touch.move.x;
    input.my=(keys.has('KeyS')||keys.has('ArrowDown')?1:0)-(keys.has('KeyW')||keys.has('ArrowUp')?1:0)+touch.move.y;
    input.fire=mouseDown||touch.aiming;input.autoAim=false;
    if(touch.aiming&&game.player){input.aimX=game.player.x+touch.aim.x*460;input.aimY=game.player.y+touch.aim.y*460;}
  }
