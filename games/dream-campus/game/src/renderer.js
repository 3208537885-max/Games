(function(root){
  ...
  class Renderer{
    ...
    drawEnemyAura(e){const c=this.c,t=this.now,pulse=.5+.5*Math.sin(t*5+e.id);c.save();c.translate(e.x,e.y);
      if(e.elite){c.globalAlpha=.34+.22*pulse;c.strokeStyle='#f4cd72';c.lineWidth=2;c.setLineDash([5,5]);c.beginPath();c.arc(0,-18,e.r+13+pulse*4,0,TAU);c.stroke();c.setLineDash([]);A.line(c,-8,-e.r*2-13,0,-e.r*2-20,'#f4cd72',3);A.line(c,0,-e.r*2-20,8,-e.r*2-13,'#f4cd72',3);}
      if(e.boss){c.globalAlpha=.13+.08*pulse;c.strokeStyle=e.color;c.lineWidth=7;c.beginPath();c.arc(0,-28,e.r+24+pulse*8,0,TAU);c.stroke();}
      if(e.windup>0){const r=e.r+10+e.windup*18;c.globalAlpha=.7;c.strokeStyle='#ff9c83';c.lineWidth=3;c.beginPath();c.arc(0,-16,r,0,TAU);c.stroke();for(let i=0;i<4;i++){const a=i*Math.PI/2+t*1.5;A.line(c,Math.cos(a)*(r+9),-16+Math.sin(a)*(r+9),Math.cos(a)*r,-16+Math.sin(a)*r,'#ffd09b',2);}}
      if(e.chargeTime>0){c.globalAlpha=.25+.18*pulse;const a=Math.atan2(e.chargeY||0,e.chargeX||1);c.rotate(a);for(let i=0;i<3;i++)A.line(c,-e.r-12-i*18,-10+i*8,-e.r-34-i*18,-10+i*8,'#ffb58d',5-i);}
      if(e.wet>0){c.globalAlpha=.55;c.strokeStyle='#82dfe5';c.lineWidth=3;c.beginPath();c.arc(0,3,e.r+7,t*2,t*2+4.6);c.stroke();}
      if(e.slow>0){c.globalAlpha=.6;c.strokeStyle='#b9f4ed';c.lineWidth=2;for(let i=0;i<6;i++){const a=i*TAU/6-t;A.line(c,Math.cos(a)*(e.r+8),-18+Math.sin(a)*(e.r+8),Math.cos(a)*(e.r+15),-18+Math.sin(a)*(e.r+15),'#b9f4ed',2);}}
      if(e.burn>0){c.globalAlpha=.75;for(let i=0;i<3;i++){const x=(i-1)*10+Math.sin(t*7+i)*3,y=-e.r*2-5-Math.abs(Math.sin(t*6+i))*10;c.fillStyle=i===1?'#ffd27b':'#ef8c65';c.beginPath();c.moveTo(x-5,y+9);c.quadraticCurveTo(x-7,y,x,y-8);c.quadraticCurveTo(x+8,y,x+5,y+9);c.fill();}}
      if(e.stun>0){c.globalAlpha=.8;for(let i=0;i<3;i++){const a=t*4+i*TAU/3,x=Math.cos(a)*(e.r+10),y=-e.r*2-12+Math.sin(a)*5;c.fillStyle='#ffe39a';c.beginPath();for(let j=0;j<8;j++){const q=j*Math.PI/4-Math.PI/2,r=j%2?2.5:6;c.lineTo(x+Math.cos(q)*r,y+Math.sin(q)*r);}c.fill();}}
      c.restore();
    }
    drawBullet(b){const c=this.c;if(D.renderHooks?.bullet?.(c,b,this.game,this.now))return;c.save();c.translate(b.x,b.y);c.rotate(b.a);
      ...
      else if(b.w.bulletStyle==='coin'){c.rotate(this.now*14+b.id);A.line(c,-b.r*3,0,-b.r*1.2,0,'#f6d17988',3);A.ellipse(c,0,0,b.r*1.25,b.r*1.25,'#d99c42');A.ellipse(c,0,0,b.r*.82,b.r*.82,'#ffe08a');c.strokeStyle='#fff2b7';c.lineWidth=1.5;c.beginPath();c.arc(0,0,b.r*.52,0,TAU);c.stroke();}
      else if(b.w.bulletStyle==='chalk'){A.line(c,-b.r*3.2,0,-b.r*.8,0,'#f1efd088',2);A.box(c,-b.r*.7,-2,b.r*2.3,4,'#f8f3d6',1,'#c9c7ad',1);for(let i=0;i<3;i++){c.globalAlpha=.35;A.ellipse(c,-b.r*(1.2+i*.7),Math.sin(this.now*13+i)*3,1.5,1.5,'#f7f1d5');}}
      else if(b.w.bulletStyle==='frost'){c.globalAlpha=.82;A.line(c,-b.r*2.8,0,-b.r*.8,0,'#9ce7e788',b.r*.45);c.rotate(this.now*5+b.id);c.strokeStyle='#cafff7';c.lineWidth=2;for(let i=0;i<3;i++){c.rotate(Math.PI/3);A.line(c,-b.r,0,b.r,0,'#baf4ef',2);}A.ellipse(c,0,0,b.r*.45,b.r*.45,'#efffdc');}
      else if(b.w.bulletStyle==='smoke'){c.rotate(-b.a);for(let i=0;i<3;i++){c.globalAlpha=.28+i*.18;A.ellipse(c,-8+i*7,Math.sin(this.now*7+i)*3,b.r*(1.25-i*.14),b.r*(.9-i*.08),i===2?'#f1eed0':'#b9cbb5');}}
      else if(b.w.bulletStyle==='ticket'){A.line(c,-b.r*4,0,-b.r*1.8,0,'#ffc98788',4);A.box(c,-b.r*1.4,-b.r*.65,b.r*2.8,b.r*1.3,'#f5cf8a',2,'#694f46',1.5);A.line(c,-2,-b.r*.45,-2,b.r*.45,'#9b6d5d',1);A.line(c,4,-b.r*.35,b.r*.9,-b.r*.35,'#9b6d5d',1);}
      else if(b.w.bulletStyle==='notification'){A.line(c,-b.r*3,0,-b.r*1.2,0,'#f3a8bb77',3);A.box(c,-b.r*1.35,-b.r,b.r*2.7,b.r*2,'#f4e8cb',4,'#6c5a64',1.5);A.line(c,-b.r*.8,-2,b.r*.55,-2,'#8b8090',1.5);A.line(c,-b.r*.8,3,b.r*.25,3,'#8b8090',1.5);A.ellipse(c,b.r*.9,-b.r*.82,b.r*.55,b.r*.55,'#ec6f78');}
      else if(b.w.bulletStyle==='formula'){c.rotate(this.now*8*(b.returning?-1:1));A.line(c,-b.r*3,0,-b.r*1.2,0,'#eee1b377',3);A.box(c,-b.r*1.35,-b.r,b.r*2.7,b.r*2,'#f2e7c4',2,'#75866f',1);c.fillStyle='#795d69';c.font=`bold ${Math.max(8,b.r*1.5)}px serif`;c.textAlign='center';c.textBaseline='middle';c.fillText('Σ',0,1);}
    ...
    drawEffect(e){const c=this.c,k=Math.max(0,e.t/e.max);c.save();
      switch(e.type){
        ...
        case 'impact':{const p=1-k,r=e.r*(.45+p*.9),rng=new D.RNG(e.seed);c.translate(e.x,e.y);c.rotate(e.a||0);c.globalAlpha=k;c.strokeStyle=e.color;c.lineWidth=e.style==='frost'?2:3;c.beginPath();c.arc(0,0,r,0,TAU);c.stroke();const n=e.style==='notification'?4:e.style==='formula'?6:5;for(let i=0;i<n;i++){const a=TAU*i/n+(rng.next()-.5)*.3,len=r+5+rng.next()*8;A.line(c,Math.cos(a)*r,Math.sin(a)*r,Math.cos(a)*len,Math.sin(a)*len,e.color,2);}if(e.style==='frost'){for(let i=0;i<4;i++){c.rotate(Math.PI/4);A.line(c,-r*.55,0,r*.55,0,'#eaffdf',1.5);}}else if(e.style==='notification')A.box(c,-10,-16,20,11,'#f8e9cd',3,'#ed7982',1);break;}
        case 'enemyDeath':{const p=1-k,r=e.r*(.8+p*1.8),rng=new D.RNG(e.seed);c.translate(e.x,e.y);c.globalAlpha=k*.85;c.strokeStyle=e.color;c.lineWidth=e.boss?7:e.elite?4:3;c.beginPath();c.arc(0,0,r,0,TAU);c.stroke();if(['charge','strafe'].includes(e.style)){c.rotate(e.a||0);for(let i=-2;i<=2;i++)A.line(c,-r*.3,i*7,-r*(1.1+rng.next()*.7),i*9,e.color,3);}else if(e.style==='bomb'){c.strokeStyle='#ffbd7e';c.lineWidth=5;c.beginPath();c.arc(0,0,r*1.35,0,TAU);c.stroke();}else if(e.style==='teleport'){for(let i=0;i<8;i++)A.box(c,(rng.next()-.5)*r*2,(rng.next()-.5)*r*2,5,5,'#9be9df',1);}else if(e.style==='summon'){c.setLineDash([8,7]);c.beginPath();c.arc(0,0,r*1.25,0,TAU);c.stroke();c.setLineDash([]);}else{for(let i=0;i<(e.boss?16:8);i++){const a=TAU*i/(e.boss?16:8)+(rng.next()-.5)*.25;A.line(c,Math.cos(a)*r*.6,Math.sin(a)*r*.6,Math.cos(a)*r*1.35,Math.sin(a)*r*1.35,e.color,e.boss?4:2);}}break;}
