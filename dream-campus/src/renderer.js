(function(root){
  ...
  class Renderer{
    ...
    render(now){
      ...
      for(const e of g.effects)if(['warningLine','spawn','dash','afterimage','pulse','slash'].includes(e.type))this.drawEffect(e);
      ...
      for(const o of g.orbits){c.save();c.globalAlpha=Math.min(1,o.life);A.icon(c,D.WEAPONS[o.weapon].icon,o.x,o.y-8,33,o.color);c.restore();}
      for(const e of g.effects)if(!['warningLine','spawn','dash','afterimage','pulse','slash','victory'].includes(e.type))this.drawEffect(e);
    ...
    drawBullet(b){const c=this.c;if(D.renderHooks?.bullet?.(c,b,this.game,this.now))return;c.save();c.translate(b.x,b.y);c.rotate(b.a);
      if(!b.friendly){A.ellipse(c,0,1,b.r+3,b.r+3,'#371e20');A.ellipse(c,0,0,b.r,b.r,'#e6837c');A.ellipse(c,-1,-1,b.r*.49,b.r*.49,'#ffdfa8');}
